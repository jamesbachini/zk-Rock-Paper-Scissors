...
// Unit tests for the zkrps contract using a simple mock GameHub.
...
use crate::{Error, ZkrpsContract, ZkrpsContractClient};
...
fn setup_test() -> (
    ...
    // Deploy zkrps with admin and GameHub address
    let contract_id = env.register(ZkrpsContract, (&admin, &hub_addr));
    let client = ZkrpsContractClient::new(&env, &contract_id);

    // Register zkrps as a whitelisted game (mock does nothing)
