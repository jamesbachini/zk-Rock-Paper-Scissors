...
pub fn load_vk_from_bytes(bytes: &Bytes) -> Option<VerificationKey> {
    ...
    const LEGACY_LEN: usize = HEADER_WORDS * 8 + NUM_POINTS * 64;
    const EXTENDED_LEN: usize = LEGACY_LEN + 4;
    let len = bytes.len() as usize;
    if len != LEGACY_LEN && len != EXTENDED_LEN {
        return None;
    }
    ...
    if len == EXTENDED_LEN {
        // bb.js emits an extra u32 after the header in some versions.
        idx += 4;
    }
